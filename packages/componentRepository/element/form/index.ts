    ...
    uiUtil.createInput('label-width'),
      {
        ...
        editConfig:function(d){return  [
          ...
          uiUtil.createSelect('~controllerType',['input','input-number','select','switch','container']),
          uiUtil.createSelect('_type',['text','textarea'],undefined,{'~show': computed(() =>  (unref(d)['~controllerType'])=='input')}),
          uiUtil.createSwitch('_controls','Show controls',{'~show': computed(() =>  (unref(d)['~controllerType'])=='input-number'),'~default':true}),
