import { ref, reactive, computed, watch, isRef, isReactive } from 'vue'
...
export default function propsEditorData(context) {
  ...
  //
  const configBasic = ref(pageProps())

  watch(
    () => context.choosedManager.getChoosed(),
    () => {
      //console.log('configBasic is calculated')
      //
      const choosed = context.choosedManager.getChoosed()

      if (!choosed || !choosed.key) {
        //
        configBasic.value = pageProps()
        return
      ...
      if (!componentConfig) {
        configBasic.value = promptMsg('No component config is found by type:' + choosed.type)
        return
      }
      //
      if (!componentHasConfig(componentConfig.editor)) {
        configBasic.value = promptMsg('This component does not need to config')
        return
      }

      //
      configBasic.value = formTransform(
        componentConfig.editor,
        choosed.config?.props || {},
        context,true
      )
    },
    {
        ...
        return [
          {
            ...
            '~prop': 'name',
            'allow-create': true,
            '~options': componentConfig.events || []
          },
          {
            '~component': 'el-select',
            '~label': 'Handle mode',
          ...
          },
          {
            ...
            '~show': computed(
              () =>
                d.value.mode == 'openPage' &&
                (d.value.openPageType == 'dialog' || d.value.openPageType == 'drawer')
            )
          },
          {
            '~component': 'el-switch',
