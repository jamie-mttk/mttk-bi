<template>
    <div class="EntityNode">
        ...
        <div class="content" @click="handleCommand('fields')">
            <SmartView :value="props.modelValue?.entity?.name" :length="16"> </SmartView>
        </div>
        <div class="suffix">
            <el-dropdown trigger="click"  style="height:100%;" @command="handleCommand">
               <span style="height:100%;"> <lc-icon icon="mdiDotsHorizontal"  style="height:100%;"></lc-icon></span>
                <template #dropdown>
                    <el-dropdown-menu>
                        <el-dropdown-item command="basic">{{ $t('bi.plugin.modelEditor.single.entityTree.menu.basic') }}</el-dropdown-item>
                        <el-dropdown-item command="fields">{{ $t('bi.plugin.modelEditor.single.entityTree.menu.fields') }}</el-dropdown-item>
                        <el-dropdown-item command="delete" divided
                            v-if="props.modelValue.children.length == 0">{{ $t('_._.del') }}</el-dropdown-item>
                    </el-dropdown-menu>
                </template>
            </el-dropdown>
        </div>
