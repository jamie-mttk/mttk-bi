<template>
  <el-dialog
    ...
    :title="$t('bi.plugin.modelEditor.single.entityTree.edit.title')"
    width="75%"
  ...
  >
    ...
    <template #footer>
      <span class="dialog-footer">
        <el-switch
          v-model="useRemarkAsName"
          style="margin-right: 64px"
          :active-text="$t('bi.plugin.modelEditor.single.entityTree.edit.useRemark')"
        />
        <el-button @click="dialogVisible = false">{{ $t('_._.cancel') }}</el-button>
        <el-button type="primary" @click="handleConfirm"> {{ $t('_._.ok') }} </el-button>
      </span>
    </template>
  </el-dialog>
</template>
...
async function handleConfirm() {
  nodeEditBasicRef.value.validateFunc()(function (valid1) {
    if (valid1) {
        const joinValidate=nodeEditJoinRef.value.validateFunc()

      if (!joinValidate) {
        handleUpdate()
      } else {
        joinValidate(function (valid2) {
          //
          if (valid2) {
            handleUpdate()
          }
...
function handleUpdateAdd() {
  ...
  //Add all columns
  loadEntityColumnsPromise(globalContext.request, modelSaved._id, dataBasic.value).then(function (
    response
  ) {
