<template>
    <div>
        <div style="font-weight: 700;margin-left:20px;margin-bottom: 10px;">{{$t('bi.plugin.modelEditor.single.entityList.tableOrView')}}</div>
        <div class="table-button" :draggable="true" @dragstart="handleDragstartSQL($event)">
            <lc-icon icon="mdiDatabaseEditOutline"></lc-icon>
            {{$t('bi.plugin.modelEditor.single.entityList.insertSQL')}}</div>
