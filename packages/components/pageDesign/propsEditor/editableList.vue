  ...
  <div style="width:100%;">
    <div v-if="mode == 'list'">
      ...
      <!-- <draggable tag="ul" :list="modelValue" class="list-group" handle=".handle" item-key="name"> -->
        <VueDraggable tag="ul" :modelValue="props.modelValue" @update:modelValue="(value)=>emit('update:modelValue',value)" class="list-group" handle=".handle" item-key="key">
    
          <li class="list-group-item"  v-for="(item, index) in props.modelValue" :key="item.key">
    ...
    <div v-if="mode != 'list'">
      <el-page-header title="back" @back="goBack" style="padding:10px;background-color: var(--el-fill-color-extra-light);">
        ...
        <MttkWrapComp :config="editorConfigTranformed" style="margin:4px;" ref="tableEditor"></MttkWrapComp>
      </el-page-header>
    </div>
  </div>
</template>
